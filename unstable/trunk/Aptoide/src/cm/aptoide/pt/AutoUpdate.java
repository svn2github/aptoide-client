...
public class AutoUpdate extends AsyncTask<Void, Void, AutoUpdate.AutoUpdateInfo> {
    ...
    protected void onPostExecute(AutoUpdateInfo autoUpdateInfo) {
        ...
        if(autoUpdateInfo!=null){
            ...
            try {
                if(vercode > activity.getPackageManager().getPackageInfo(packageName,0).versionCode || AptoideConfiguration.getInstance().isAlwaysUpdate()){
    ...
    private void requestUpdateSelf(final AutoUpdateInfo autoUpdateInfo) {
        ...
        updateSelfDialog.setMessage(activity.getString(R.string.update_self_msg, ApplicationAptoide.MARKETNAME));
