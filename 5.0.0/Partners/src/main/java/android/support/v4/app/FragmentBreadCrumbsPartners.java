 ...
 */
public class FragmentBreadCrumbsPartners extends ViewGroup
    ...
    public FragmentBreadCrumbsPartners(Context context) {
        this(context, null);
    }

    public FragmentBreadCrumbsPartners(Context context, AttributeSet attrs) {
        this(context, attrs, R.style.Widget_HanselAndGretel_FragmentBreadCrumb);
    }

    public FragmentBreadCrumbsPartners(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setFragment(Fragment a) {
        mActivity = a;
        mInflater = (LayoutInflater)a.getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mContainer = (LinearLayout)mInflater.inflate(
                R.layout.hag__fragment_bread_crumbs,
                this, false);
        addView(mContainer);
        a.getChildFragmentManager().addOnBackStackChangedListener(this);
        updateCrumbs();
    }
    ...
    private BackStackRecord createBackStackEntry(CharSequence title, CharSequence shortTitle) {
        ...
        final BackStackRecord entry = new BackStackRecord(
                (FragmentManagerImpl) mActivity.getChildFragmentManager());
    ...
    void updateCrumbs() {
        FragmentManager fm = mActivity.getChildFragmentManager();
            ...
            if (v.getTag() instanceof BackStackEntry) {
                ...
                if (bse == mParentEntry) {
                    ...
                    if (bse == mTopEntry) {
                        ...
                        mActivity.getChildFragmentManager().popBackStack();
                    } else {
                        mActivity.getChildFragmentManager().popBackStack(bse.getId(), 0);
                    }
                }
            }
        }
