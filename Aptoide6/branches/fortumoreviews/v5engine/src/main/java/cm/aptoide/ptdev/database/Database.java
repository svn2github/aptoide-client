...
*/
    ...
    public long getRepoId(String reponame){
        ...
        final Cursor servers = getServers();

        for(servers.moveToFirst(); !servers.isAfterLast(); servers.moveToNext()){

            if(servers.getString(servers.getColumnIndex(Schema.Repo.COLUMN_NAME)).equals(reponame)){
                final long aLong = servers.getLong(servers.getColumnIndex(Schema.Repo.COLUMN_ID));
                servers.close();
                return aLong;
            }
