...
import cm.aptoidetv.pt.SecurePrefs.Base64;
import cm.aptoidetv.pt.Utils;
...
public class AptoideUtils {
    ...
    private final static String gles = HWSpecifications.getGlEsVer(AppTV.getContext());
    private final static int screen = HWSpecifications.getScreenSize(AppTV.getContext());
    private final static String screenSpec = HWSpecifications.getNumericScreenSize(AppTV.getContext())+ "/" + HWSpecifications.getDensityDpi(AppTV.getContext());
    ...
    public static boolean isLoggedIn(Context context) {
        ...
        return manager.getAccountsByType(AppTV.getConfiguration().getAccountType()).length != 0;
    }
    public static Account getUser(Context context) {
        AccountManager manager = AccountManager.get(context);

        Account[] accounts = manager.getAccountsByType(AppTV.getConfiguration().getAccountType());
    ...
    public static boolean isLoggedInOrAsk(Activity activity) {
        ...
        if (manager.getAccountsByType(AppTV.getConfiguration().getAccountType()).length == 0) {
            manager.addAccount(AppTV.getConfiguration().getAccountType(),
                    LoginActivity.AUTHTOKEN_TYPE_FULL_ACCESS, null, null, activity,null, null);
...
/*
    ...
    public static SharedPreferences getSharedPreferences(){
        return PreferenceManager.getDefaultSharedPreferences(AppTV.getContext());
    }
    ...
    public static String checkStoreUrl(String uri_str){
        ...
        if (!uri_str.contains(".")) {
            uri_str = uri_str.concat(".store.AppTV.com");
        }

        uri_str = RepoUtils.formatRepoUri(uri_str);

        if (uri_str.contains("bazaarandroid.com")) {
            uri_str = uri_str.replaceAll("bazaarandroid\\.com", "store.AppTV.com");
        }
    ...
    public static void toastError(List<Error> errors) {
        for (Error error: errors){
            ...
            try{
                localizedError = AppTV.getContext().getString(Errors.getErrorsMap().get(error.getCode()));
            }catch (NullPointerException ignored){}
            if(localizedError==null){
                localizedError = error.getMsg();
            }
            if (localizedError!=null) Toast.makeText(AppTV.getContext(), localizedError, Toast.LENGTH_LONG).show();
        }
    }

    public static void toast(String text) {
        Toast.makeText(AppTV.getContext(), text, Toast.LENGTH_LONG).show();
    }
    ...
    protected static float getScreenWidthInDip() {
        WindowManager wm = ((WindowManager) AppTV.getContext().getSystemService(Context.WINDOW_SERVICE));
    ...
    public static class NetworkUtils {
        ...
        public BufferedInputStream getInputStream(String url, String username, String password, Context mctx) throws IOException {
            ...
            connection.setRequestProperty("User-Agent", Utils.getUserAgentString(mctx));
        ...
        public JSONObject getJsonObject(String url, Context mctx) throws IOException, JSONException {
            String line = null;
            InputStreamReader reader = new InputStreamReader(getInputStream(url, null, null, mctx));
        ...
        public static NetworkType getConnectionType(){
            Context context = AppTV.getContext();
...
*/
    ...
    public static class RepoUtils {
        ...
        public static String split(String repo) {
            Log.d("AppTV-RepoUtils", "Splitting " + repo);
        ...
        public static String formatRepoUri(String uri_str) {
            ...
            if (uri_str.length() != 0 && uri_str.charAt(uri_str.length() - 1) != '/') {
                uri_str = uri_str + '/';
                Log.d("AppTV-ManageRepo", "repo uri: " + uri_str);
            }
...
/*
    ...
    public static void syncInstalledApps(Context context, SQLiteDatabase db) {
        //Log.d("AppTV-InstalledSync", "Syncing");
        long startTime = System.currentTimeMillis();

        InstalledAppsHelper.sync(db, context);

        context.startService(new Intent(context, UpdatesService.class));
        Log.d("AppTV-InstalledSync", "Sync complete in " + (System.currentTimeMillis() - startTime)+"ms");
