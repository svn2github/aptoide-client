 ...
 * </pre>
 */
public class FlipDrawable extends Drawable implements Drawable.Callback, AnimatorUpdateListener {
     ...
     *                           to add flourish.
     */
    public FlipDrawable(final Drawable front, final int flipDurationMs,
            final int preFlipDurationMs, final int postFlipDurationMs, Resources res) {
        if (front == null ) {
            throw new IllegalArgumentException("Front drawable must not be null.");
        }
        mFront = front;

        mCheckmarkDrawable = new CheckmarkDrawable(res);

        mFront.setCallback(this);
        mCheckmarkDrawable.setCallback(this);
...
// Actual duration is the second half of the flip duration.
        ...
        mCheckmarkScaleAnimator = ValueAnimator.ofFloat(CHECKMARK_SCALE_BEGIN_VALUE, END_VALUE / 2)
    ...
    protected void onBoundsChange(final Rect bounds) {
        ...
        if (bounds.isEmpty()) {
            ...
            mCheckmarkDrawable.setBounds(0, 0, 0, 0);
        } else {
            mFront.setBounds(bounds);
            mCheckmarkDrawable.setBounds(bounds);
        }
    }
    ...
    public void draw(final Canvas canvas) {
        ...
        final Drawable inner = getSideShown() /* == front */ ? mFront : mCheckmarkDrawable;
    ...
    public void setAlpha(final int alpha) {
        ...
        mCheckmarkDrawable.setAlpha(alpha);
    }

    @Override
    public void setColorFilter(final ColorFilter cf) {
        mFront.setColorFilter(cf);
        mCheckmarkDrawable.setColorFilter(cf);
    }

    @Override
    public int getOpacity() {
        return resolveOpacity(mFront.getOpacity(), mCheckmarkDrawable.getOpacity());
    }

    @Override
    protected boolean onLevelChange(final int level) {
        return mFront.setLevel(level) || mCheckmarkDrawable.setLevel(level);
    }
...
// Call from super's constructor. Not yet initialized.
        ...
        mCheckmarkDrawable.setScaleAnimatorValue(side ? CHECKMARK_SCALE_BEGIN_VALUE : END_VALUE / 2);
