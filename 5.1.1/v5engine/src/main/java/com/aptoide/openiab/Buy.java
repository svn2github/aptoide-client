...
public class Buy extends ActionBarActivity {
	...
	String token ;
    int paymentTypeId;
	String url = "http://webservices.aptoide.com/" + /*Aptoide.getConfiguration().getWebServicesUri() +*/ "webservices/productPurchaseAuthorization";
...
//	String urlRedirect="http://www.sandbox.paypal.com/webscr?cmd=_ap-payment&paykey=";
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        String params = token+"/"+paymentTypeId+"/json";
        send(url, params);
		...
		Thread t = new Thread(){
			public void run() {
				...
				try {
                    ...
                    Log.d("Paypal", "Url: " + request.getURI());
	...
	public PayPalPreapproval Preapproval() {
		...
		preapproval.setMemo("Purchase of InAppPurchase");
		preapproval.setMerchantName("Aptoide");
...
//		String params = token+"/"+userMail+"/" + repo +"/"+apkid+"/"+versionName+"/pay/json";
        String params = apiVersion+"/"+token+"/" + aptoideProductId +"/pay/"+paymentTypeId+"/json";
